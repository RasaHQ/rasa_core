...
class TrackerFeaturizer(object):
    ...
    def _featurize_topics(
        self,
        trackers_as_topics: List[List[Optional[Text]]],
        domain: Domain
    ) -> np.ndarray:
        """Create topic array for training"""

        topics = []
        for tracker_topics in trackers_as_topics:

            if len(trackers_as_topics) > 1:
                tracker_topics = self._pad_states(tracker_topics)

            story_labels = [self.state_featurizer.topic_as_one_hot(topic,
                                                                   domain)
                            for topic in tracker_topics]

            topics.append(story_labels)
