...
class TrackerFeaturizer(object):
    ...
    def featurize_trackers(self,
        ...
        return DialogueTrainingData(X, y, true_lengths, histories=trackers_hist, trackers=trackers_full)
...
class MaxHistoryTrackerFeaturizer(TrackerFeaturizer):
            ...
            for evdx, event in enumerate(tracker.applied_events()):
                if isinstance(event, ActionExecuted):
                    if not event.unpredictable:
                        ...
                        earliest_event = evdx - self.max_history
                        latest_event = evdx
                        # keep only stories which happen within the previous max_history steps
    ...
    def _remove_duplicate_states(
        ...
        for (tracker_states,
            ...
            if hashed not in hashed_featurizations_set:
                hashed_featurizations_set.add(hashed)
